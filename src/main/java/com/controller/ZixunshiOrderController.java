...
//                            zixunshiOrderEntity.setZixunshiOrderUuidNumber(data.get(0));                    //预约流水号 要改的
...
//                            zixunshiOrderEntity.setZixunshiOrderTruePrice(data.get(0));                    //实付金额 要改的
//                            zixunshiOrderEntity.setYuyueTime(sdf.parse(data.get(0)));          //预约日期 要改的
//                            zixunshiOrderEntity.setShijianduanTypes(Integer.valueOf(data.get(0)));   //预约时间段 要改的
//                            zixunshiOrderEntity.setZixunshiOrderTypes(Integer.valueOf(data.get(0)));   //预约状态 要改的
    ...
    */
    ...
    public R add(@RequestBody ZixunshiOrderEntity zixunshiOrder, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,zixunshiOrder:{}",this.getClass().getName(),zixunshiOrder.toString());
            ...
            double balance = yonghuEntity.getNewMoney() - zixunshiEntity.getGuahaofei();//余额
            if(balance<0)
                return R.error(511,"余额不够支付");
            zixunshiOrder.setZixunshiOrderTypes(1); //设置订单状态为已支付
            zixunshiOrder.setZixunshiOrderTruePrice(zixunshiEntity.getGuahaofei()); //设置实付价格
    ...
    */
    ...
    public R refund(Integer id, HttpServletRequest request){
        ...
        String role = String.valueOf(request.getSession().getAttribute("role"));
            ...
            ZixunshiOrderEntity zixunshiOrder = zixunshiOrderService.selectById(id);
            Integer zixunshiId = zixunshiOrder.getZixunshiId();
            if(zixunshiId == null)
                return R.error(511,"查不到该咨询师");
            ZixunshiEntity zixunshiEntity = zixunshiService.selectById(zixunshiId);
            if(zixunshiEntity == null)
                return R.error(511,"查不到该咨询师");
            Double zixunshiNewMoney = zixunshiEntity.getGuahaofei();
            if(zixunshiNewMoney == null)
                return R.error(511,"咨询师价格不能为空");
            ...
            Double zhekou = 1.0;
                ...
                //计算所获得积分
                Double buyJifen = 0.0;
                yonghuEntity.setNewMoney(yonghuEntity.getNewMoney() + zixunshiOrder.getZixunshiOrderTruePrice()); //设置金额
            ...
            zixunshiOrder.setZixunshiOrderTypes(2);//设置订单状态为取消预约
            zixunshiOrderService.updateById(zixunshiOrder);//根据id更新
            yonghuService.updateById(yonghuEntity);//更新用户信息
            zixunshiService.updateById(zixunshiEntity);//更新订单中咨询师的信息
            return R.ok();
    }
    ...
    public R deliver(Integer id ){
        ...
        ZixunshiOrderEntity  zixunshiOrderEntity = new  ZixunshiOrderEntity();;
        zixunshiOrderEntity.setId(id);
        zixunshiOrderEntity.setZixunshiOrderTypes(3);
        boolean b =  zixunshiOrderService.updateById( zixunshiOrderEntity);
        if(!b){
            return R.error("已使用出错");
        }
        return R.ok();
    }
